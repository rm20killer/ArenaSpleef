...
public class SpleefArena extends Arena {
    ...
    public void onInteract(PlayerInteractEvent event) {
        ...
        if (meta.getPersistentDataContainer().has(ArenaSpleef.getInstance().getSpleefItemKey(), PersistentDataType.BOOLEAN)) {
    ...
    public void onTntPrime(TNTPrimeEvent event) {
        ...
        if (event.getCause() == TNTPrimeEvent.PrimeCause.PROJECTILE) {
            ...
            TNTPrimed tntEntity = event.getBlock().getWorld().spawn(event.getBlock().getLocation().toCenterLocation(), TNTPrimed.class, tnt -> {
